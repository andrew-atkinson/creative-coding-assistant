...
export const POST: RequestHandler = async ({ request, getClientAddress }) => {
  ...
  const course = await store.loadCourse(course_slug);
  if (!course || course.status !== 'published') {
    return json({ error: 'not_found' }, { status: 404, headers: cors });
  }

  const monthCount = await store.currentMonthChatCount(course.id);
  if (monthCount >= course.monthly_request_budget) {
